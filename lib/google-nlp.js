...
export async function analyzeEntities(text, { force = false, cacheTtlSeconds = null } = {}) {
  const clean = String(text || "").replace(/\s+/g, " ").trim();
  if (!clean) throw new Error("Text is required for Google NLP entity analysis");
  if (!googleNlpConfigured()) {
    const error = new Error("Google NLP is not configured");
    await recordGoogleNlpFailure(error);
    throw error;
  ...
  const version = process.env.GOOGLE_NLP_API_VERSION || "v2";
  const key = cacheKey(`google-nlp-entities-${version}`, clean);
  try {
    const result = await getOrSetCache(key, async () => {
      const payload = await callLanguage("analyzeEntities", clean);
      return {
        entities: (payload.entities || []).slice(0, 25).map(item => ({
          name: item.name,
          type: item.type,
          salience: item.salience ?? null,
          mentions: Array.isArray(item.mentions) ? item.mentions.length : null
        })),
        language: payload.languageCode || payload.language || null,
        api_version: version,
        analyzed_at: new Date().toISOString()
      };
    }, {
      ...
      staleIfErrorSeconds: Number(process.env.GOOGLE_NLP_STALE_TTL_SECONDS || 7776000),
      force,
      metadata: { type: "google-nlp-entities", api_version: version, chars: clean.length }
    });
